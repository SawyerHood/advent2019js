...
function parseInput(str) {
  return str.split("\n").map(line =>
    line.split(",").map(token => {
      switch (token[0]) {
        case "R":
          return { x: +token.substring(1), y: 0 };
        case "L":
          return { x: -token.substring(1), y: 0 };
        case "U":
          return { x: 0, y: +token.substring(1) };
        case "D":
          return { x: 0, y: -token.substring(1) };
      }
    })
...
function toSegments(wire) {
  const segments = [{ x: 0, y: 0 }];

  function getEnd() {
    return segments.length ? segments[segments.length - 1] : { x: 0, y: 0 };
  }

  for (const p of wire) {
    const end = getEnd();
    segments.push(add(end, p));
  }
...
function getOverlap([a1, a2], [b1, b2]) {
  ...
  function getPoint(a1, a2, b1, b2) {
    if (isHorizontal(a1, a2)) {
      if (a1.y >= Math.min(b1.y, b2.y) && a1.y <= Math.max(b1.y, b2.y)) {
        if (b1.x >= Math.min(a1.x, a2.x) && b1.x <= Math.max(a1.x, a2.x)) {
  ...
  return getPoint(a1, a2, b1, b2) || getPoint(b1, b2, a1, a2);
}

function isHorizontal(a1, a2) {
  return a1.y === a2.y;
}

function run() {
  const [wireA, wireB] = parseInput(INPUT).map(toSegments);
  let minDist;
  let iDist = 0;
  for (let i = 0; i < wireA.length - 1; i++) {
    let jDist = 0;
    for (let j = 0; j < wireB.length - 1; j++) {
      const point = getOverlap(
        [wireA[i], wireA[i + 1]],
        [wireB[j], wireB[j + 1]]
      );
      ...
      if (point) {
        const dist =
          getDist(wireA[i], point) + iDist + getDist(wireB[j], point) + jDist;
        if (dist !== 0 && (!minDist || dist < minDist)) {
          minDist = dist;
        }
      }

      jDist += getDist(wireB[j], wireB[j + 1]);
    }
    iDist += getDist(wireA[i], wireA[i + 1]);
  }
  console.log(minDist);
}
